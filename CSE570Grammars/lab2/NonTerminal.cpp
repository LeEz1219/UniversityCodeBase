	...
	while (bad)
	{
		...
		while (mapitr != this->Productions.end()) 
		{
			if (!inFirstMap(mapitr->first)) 
			{
				...
				{
					unsigned int j = 0;
					while (j < mapitr->second[i].size())
					{
						if (!isTerminal(mapitr->second[i][j])) 
						{
							if (inFirstMap(mapitr->second[i][j]))
							{
						...
						if (temp.find('#') == temp.end() || (mapitr->second[i].size() - 1 == j)) 
							break;
						else
							temp.erase('#');
						j++;
...
void NonTerminal::findFollow()
{
	...
	while (mapitr1 != this->Productions.end()) 
	{
		char TARGET = mapitr1->first; 
...
set<char> NonTerminal::follow(char c, set<char> t, map<char, vector<string> >::iterator i)
{
	...
	for (unsigned int j = 0; j < i->second.size(); j++)  
		if (isFound(i->second[j], c)) 
			if (isTerminal(findRHS(c, i->second[j])) && findRHS(c, i->second[j]) != ' ') 
			{
				t.insert(findRHS(c, i->second[j])); 
			}
			else if (!isTerminal(findRHS(c, i->second[j])))
				int loc = getLoc(i->second[j], c);
				mergeCSet(t, firstof(findRHS(c, i->second[j])));
				while (!isTerminal(findRHS(loc, i->second[j])) && t.find('#') != t.end()) 
				{
					t.erase('#');
...
void NonTerminal::foilMap()
{
	...
	{
		bad = false;
		for (map<char, set<char> >::iterator itr = this->FollowMap.begin(); itr != this->FollowMap.end(); itr++) 
		{
			bool smallbad = false;
			for (set<char>::iterator setitr = itr->second.begin(); setitr != itr->second.end(); setitr++) 
			{
				if (!isTerminal(*setitr))
				{
					bad = true;
					smallbad = true;
					if (*setitr == itr->first) 
					...
					else if (good.find(*setitr) == good.end())
					{
						break;
					}
					else 
...
{
	return (Productions.find(c) == Productions.end()); 
}

bool NonTerminal::inFirstMap(char c)
{
	return (FirstMap.find(c) != FirstMap.end()); 
}
