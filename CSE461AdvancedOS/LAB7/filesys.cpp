...
Filesys::Filesys(string diskname, int numberofblocks, int blocksize) : Sdisk(diskname,numberofblocks, blocksize){
	...
	string buffer;
	getblock(1, buffer);
	if(buffer[0] == '#'){ //no file system on disk
		//create ROOT
		...
		//create FAT
		fat.push_back(fatsize + 2); // first free block
		fat.push_back(1); //  ROOT
		for(int i = 2; i < getnumberofblocks(); ++i){ // FAT itself
			i>fatsize+1 ?fat.push_back(i+1):fat.push_back(1);
		}
//		for(int i = fatsize+2; i < getnumberofblocks(); ++i){ // data
//			fat[i] = i+1;
//		}
...
int Filesys::addblock(string file, string buffer){
	//add block to end of file and returns block number
	int allocate = fat[0];
	if(allocate == 0){
	...
	if(fblock == -1){ // file does not exist.
		return 0;
	}
	if(fblock == 0){ //empty file
		for (int i = 0; i < filename.size(); ++i){
			if(filename[i]==file){
...
int Filesys::delblock(string file, int blocknumber){
	...
	int fblock = getfirstblock(file);
	if(fblock == blocknumber){//if deleting the first block of the file
		for(int i = 0; i < filename.size(); ++i){
			if(filename[i] == file){
		...
		//link the previous block of block blocknumber to the next block of block blocknumber
...
vector<string> Filesys::block(string s, int b){
	int numofblocks; //the number of blocks the string s will take up
	if(s.length()%b == 0){
